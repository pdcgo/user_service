...
func (s *v2UserServiceImpl) UserList(
	...
	// Resolve the set of users to return.
	q := db.Model(&user_models.User{})

	if teamID != 0 {
		// members of the given team, optionally filtered by their role there.
		q = q.Joins("JOIN user_team_roles utr ON utr.user_id = users.id AND utr.team_id = ?", teamID)
		if req.Msg.Role != role_base.Role_ROLE_UNSPECIFIED {
			q = q.Where("utr.role = ?", uint(req.Msg.Role))
		}
	...
	if err := q.Order("users.id ASC").Find(&users).Error; err != nil {
		return nil, err
	}
	...
	resp := &user_iface.UserListResponse{Users: make([]*user_iface.UserMapItem, 0, len(users))}
	for _, u := range users {
